...
PacketBase::PacketBase(Buffer_ptr buffer) {
    ...
    if (m_Length & PACKET_TYPE_COMPRESSED) {
        m_Compressed = true;
        byte* bodyBytesDecompressed = new byte[(m_Buffer->Length() - PACKET_HEADER_SIZE) + (m_Buffer->Length() - PACKET_HEADER_SIZE) / 16 + 64 + 3];
        lzo_uint dstLen;
...
Buffer_ptr PacketBase::GetProcessedBuffer() {
	...
	if (m_Compressed) {
        ...
        byte* dst = new byte[(m_Buffer->Length() - PACKET_HEADER_SIZE) + (m_Buffer->Length() - PACKET_HEADER_SIZE) / 16 + 64 + 3];
        lzo_uint dstLen;
